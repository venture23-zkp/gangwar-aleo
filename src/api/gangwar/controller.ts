...
export const gangwarController: GangwarController = {
  ...
  sign: async (req, res) => {
    const { owner, privateKey, viewKey, character, s, k, validityTimestamp } = req.body;
    const signature = await leo.gangwar.sign(privateKey, viewKey, character, s, k, validityTimestamp);
    res.send({ signature });
  },
